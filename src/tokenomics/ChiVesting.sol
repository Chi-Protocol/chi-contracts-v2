...
contract ChiVesting is IChiVesting, OwnableUpgradeable {
    ...
    function updateEpoch(uint256 chiEmissions, uint256 stETHrewards) external onlyRewardController {
        ...
        epoch.cumulativeStETHRewardPerShare =
            epochs[currentEpoch - 1].cumulativeStETHRewardPerShare + stETHRewardPerShare;
    ...
    function availableChiWithdraw(address account) public view returns (uint256) {
        VestingData storage vesting = vestingData[account];
        return
            vesting.unlockedChi + _availableUnlockFromTo(vesting.lastWithdrawnEpoch, currentEpoch - 1, vesting.shares);
    }

    // includes available amount in `toEpoch`
    function _availableUnlockFromTo(uint256 fromEpoch, uint256 toEpoch, uint256 shares)
        internal
        view
        returns (uint256)
    {
        uint256 availableWithdrawPerShare =
            epochs[toEpoch].cumulativeUnlockedPerShare - epochs[fromEpoch].cumulativeUnlockedPerShare;
